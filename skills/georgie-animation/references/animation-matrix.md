...
- Full-silhouette horizontal drift: at most 1.5 pixels in `running` and 1 pixel in `review`. Symmetric fur motion may change the silhouette without moving the body.
...
| 7 | `running` | 6 | Codex is actively working | tail center, quarter sweep, half sweep, peak, half sweep, quarter sweep | A continuous restrained tail wag. The sequence is palindromic, so the three system repeats have no reset frame. Feet, torso, head, muzzle, drop ears, scale, and baseline stay locked. No lateral drift, nod, roll, slide, wink, or paw motion. |
